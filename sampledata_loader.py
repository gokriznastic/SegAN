...
class LITS(torch.utils.data.Dataset):
    ...
    #     try:
    #         image = self.transform(Image.fromarray(image))
    #         gtMask = self.transform(Image.fromarray(gtMask))
    #     except:
    #         print("[!] Transform is invalid.")
...
################################################
# for i in range(75):
#     gtMask = imread('/home/bharat_m/Desktop/krishna/SegAN/preprocessed/train/masks/segmentation-0-{}.tiff'.format(i))
#     gtMask = split(gtMask)
#     # print(gtMask.shape)
#     print(np.unique(gtMask[:,:,0]), np.unique(gtMask[:,:,1]))